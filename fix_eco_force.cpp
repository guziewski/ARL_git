...
FixECOForce::FixECOForce(LAMMPS *lmp, int narg, char **arg) :
  ...
  memory->create(order, nmax, 5, "eco/force:order");
  ...
  for (int i = 0; i < atom->nlocal; i++)
    order[i][0] = order[i][1] = order[i][2] = order[i][3] = order[i][4] = 0.0;
}
...
void FixECOForce::post_force(int vflag) {
  ...
  added_energy = 0.0;
  for (ii = 0; ii < inum; ii++) {
    order[ii][0] = order[ii][1] = order[ii][2] = order[ii][3] = order[ii][4] = 0;
  }
...
//communicate to acquire forcePrefact data for ghost atoms
  ...
  for (ii = 0; ii < inum; ii++) {  //forces
    i = ilist[ii];
    order[i][2] = order[i][3] = order[i][4] = 0.0;
    ...
    if (forcePrefact[i]) {
      for (jj = 0; jj < jnum; jj++) {
	    ...
	    order[i][2] += dijx * forcemag1;
            order[i][3] += dijy * forcemag1;
            order[i][4] += dijz * forcemag1;
          }  //alpha
        } // atom k
      }  //atom j
    } // speed optimization
      ...
      if (forcePrefact[j]) {
        ...
        for (kk = 0; kk < knum; kk++) {
          ...
          for (alpha = 0; alpha < 6; alpha++) {
            ...
            order[i][2] += dijx * forcemag2 + invLatVec[alpha][0] * forcemag3;
            order[i][3] += dijy * forcemag2 + invLatVec[alpha][1] * forcemag3;
            order[i][4] += dijz * forcemag2 + invLatVec[alpha][2] * forcemag3;
          }  //alpha
        } // atom k
      } // speed optimization
    }  //atom j
